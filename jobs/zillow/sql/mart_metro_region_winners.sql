WITH kpi_base AS (
  ...
  JOIN real_estate.stg_zillow_zori_sfr_smoothed_metro_us_monthly AS r
...
SELECT
  ...
  region_name,
  -- state_name,
  real_estate.us_states.state_name_full,
  home_prices,
  rent_prices,
...
LEFT JOIN real_estate.us_states
  ON state_name = real_estate.us_states.state_code
WHERE date = (SELECT MAX(date) FROM bps_momentum_valuation)
